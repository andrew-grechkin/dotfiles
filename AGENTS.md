...
Upon session init for the first time, before acknowledging the user or responding to any queries, the agent MUST:

1. Read `README.md` to acquire repository context if it exists

### Operation protocol (MANDATORY)

1. Never assume. If you don't have enough data - say that. Suggest where you can get the necessary data
2. Never loop into brute-force search for solutions, unless explicitly told to do so
3. Operate in a dialog form, explain problems if you faced them and seek for confirmation and decision making
4. Don't assume you need to make changes before you told to do so
5. Never use em and en dashes, only use simple dash or minus
...
- **Compiled Languages** (e.g., Rust, Go, C++): Use _only_ if absolutely necessary. They must provide a clear, undoubtful, and significant benefit (such as a massive performance requirement) that cannot be met by Bash or Perl. (Example: [colored-md], [update-yaml])
...
- **TUI Architecture**: Any script that implements a Text User Interface (TUI) MUST follow the Model-View-Controller (MVC) pattern. The model can be defined within the `justfile` itself (Example: [menu-pacman]) or extracted into a separate script depending on complexity (Example: [gitlab-model])
- **Arguments & Help**: Use the `argc` bash helper to leverage declarative benefits if a script takes more than 1 positional argument or requires any flags or options. (Example: [git-credential-pass-cache])
...
- **Critic**: Adhere to [perlcriticrc] (severity 1)
    ...
    - Prefer builtin `try` over `Try::Tiny`
...
- Use git non-destructive commands (log, search) without additional approval
...
[update-yaml]: https://github.com/andrew-grechkin/update-yaml
[cred]: .local/scripts/tool/cred
[gather-files]: .local/scripts/tool/gather-files
[gemini-model]: .local/scripts/model/gemini-model
[git-credential-pass-cache]: .local/scripts/tool/git-credential-pass-cache
[gitlab-model]: .local/scripts/model/gitlab-model
