...
vim.api.nvim_create_user_command('Ai', function(opts)
    ...
    local cmd = ('echo request: "%s"; echo; tee >(%s %s)'):format(opts.args, 'gemini-model streamed-prompt', opts.args)
    if end_line == -1 then
        cmd = ('echo request: "%s"; echo; cat | %s %s'):format(opts.args, 'gemini-model streamed-prompt', opts.args)
    end

    vim.cmd('vsplit')
    local bufnr = vim.api.nvim_create_buf(false, true)
    vim.api.nvim_buf_set_name(bufnr, ('gemini-model streamed-prompt %s'):format(os.time()))
