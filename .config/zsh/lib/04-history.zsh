...
# NOTE: do not export. Zsh-only shell parameter, meaningless to any other process; exporting only clutters child env
# export HISTORY_IGNORE=...
HISTORY_IGNORE='(exit( *)#|history( *)#|ai *|[bf]g *|cd *|l[alsh] *|less *|kill *|vi *|vim *)'
...
function define_histfile() {
    if [[ -n "$HOSTNAME" ]] && [[ -n "$XDG_STATE_HOME" ]]; then
        ...
        # NOTE: do not export. Child shells (bash especially) inherit it and append in a different history format,
        # corrupting the file
        # export HISTFILE
    else
...
function zshaddhistory() {
    ...
    [[ "$1" =~ "^ " ]] && return 2
    local trimmed
    if trimmed=$(perl -0777 -pXE 's/(\s | \R | (\\n))+$ | ^(\\n)+//gnxms' <<< "$1") && [[ -n "$trimmed" ]]; then
        print -Sr -- "$trimmed"
        return 1
    fi
    return 2
...
function h-disable() {
    local hf="$HISTFILE"
    fc -p "${XDG_CACHE_HOME}/zsh-history@tmp" "${HISTSIZE}" "${SAVEHIST}"
    fc -R "$hf"
...
function h-enable() {
    if [[ "$HISTFILE" == "${XDG_RUNTIME_DIR}"* ]]; then
        fc -P
    fi
}
