...
export PERLBREW_ROOT="$XDG_STATE_HOME/perlbrew/$HOSTNAME"
...
if (( $+commands[perlbrew] )); then # perlbrew is the preferred way of managing perl and libraries if installed
    enable-perlbrew
else
#   (( $+commands[cpanm] )) && activate-local-perl
    unset PERLBREW_ROOT PERLBREW_HOME
fi

export-perl5lib

unset -f enable-perlbrew export-perl5lib
