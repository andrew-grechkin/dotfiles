...
This repository contains my personal dotfiles for configuring a Linux environment that I use to customize my development
...
5. UIs built on top of CLI data models, not as a replacement for them
...
user-friendly as a graphical application, without sacrificing script-ability or the Unix principle of composition.
...
These tools ensure the developer environment is secure, hardware-backed and automated across machines.
...
- [cred]: A meta-credential helper that sits on top of Git's credential system. Used for any type of credential fetching
  in my workflow, doesn't matter what they are: tokens, encrypted at rest; oauth tokens; passwords; proprietary SSO
  tokens
...
- [menu-pacman]: A TUI for the `pacman` package manager on Arch Linux. Provides an interactive interface for discovering
  and managing system packages (see the screencast example)
- [menu-mise]: A TUI for the `mise` tool manager to search, install and manage global tool versions
- [menu-mpv-history]: A history manager for `mpv` that actively cleans and de-duplicates playback history on every run
- many more...

## Installation

I just clone this repo into the new account's home dir and run the [setup] script in the root of the repository.
